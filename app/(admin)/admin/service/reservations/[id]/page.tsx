...
interface ReservationDetail {
    ...
    // 💡 수정됨: 에어컨 크기, 설치장소 추가 및 환경(environment)을 string 배열로 변경
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="bg-white px-4 py-2 border border-slate-200 rounded-lg font-bold text-sm shadow-sm flex items-center gap-2">
                    현재 상태 :
                    <span className={`px-2 py-0.5 rounded text-xs ${reservation.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' :
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    // 💡 기존 ReservationDetailPage의  부분을 아래로 교체
                        ...
                        <div className="sm:col-span-2 mt-4 space-y-4">
                            {reservation.extraDetails.map((detail, idx) => (
                                <div key={idx} className="bg-slate-50 border border-slate-100 p-5 rounded-xl relative overflow-hidden">
                                    ...
                                    <p className="text-sm font-black text-indigo-600 mb-4 border-b border-slate-200 pb-2">#{idx + 1}번 기기 상세 정보</p>

                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-5 gap-x-4">
                                        {detail.brand && (
                                            <div><span className="text-xs text-slate-400 block mb-1">제조사(브랜드)</span><span className="text-sm font-bold text-slate-700">{detail.brand}</span></div>
                                        )}
                                        {detail.year && (
                                            <div><span className="text-xs text-slate-400 block mb-1">구입 연식</span><span className="text-sm font-bold text-slate-700">{detail.year}</span></div>
                                        )}
                                        {detail.size && (
                                            <div><span className="text-xs text-slate-400 block mb-1">크기</span><span className="text-sm font-bold text-slate-700">{detail.size}</span></div>
                                        )}
                                        {detail.location && (
                                            <div><span className="text-xs text-slate-400 block mb-1">설치 장소</span><span className="text-sm font-bold text-slate-700">{detail.location}</span></div>
                                        )}
